...
myCtrl.controller('flCtrl',['$scope','$http','$stateParams','$ionicScrollDelegate',function($scope,$http,$stateParams,$ionicScrollDelegate){
	...
	$scope.getcontent=function(){
		$http.get('http://lvmx.top/jd/?c=type&m=getDetail',{
		params:{
			type_id:$scope.currentid
		}
...
// 登录页面控制器
myCtrl.controller('loginCtrl',['$scope','$http','$ionicPopup','$state',function($scope,$http,$ionicPopup,$state){
	$scope.txt1=false;
	$scope.txt2=false;
	$scope.txt3=false;
	$scope.btnok1=false;
				...
				$.post("checknum/chk_code.php?act=char",{code:code_char},function(msg){
					if(msg==1){
						...
						if(data.code==0)
						{
							...
							sessionStorage.setItem("JDlogin_user",data_user.user);
						}
						else if(data.code==1)
						{
...
// 注册页面控制器
myCtrl.controller('registerCtrl',['$scope','$http','$state','$ionicPopup','$timeout',function($scope,$http,$state,$ionicPopup,$timeout){
